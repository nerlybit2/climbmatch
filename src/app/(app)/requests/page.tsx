...
const STATUS_CONFIG: Record<string, { bg: string; text: string; dot: string; border: string }> = {
  ...
  matched:   { bg: 'bg-[#f0f7f5]',     text: 'text-[#0a5048]',    dot: 'bg-[#0a5048]',    border: 'border-[#0a5048]/15'    },
...
export default function MyPostsPage() {
  ...
  const handleCancel = async (id: string) => {
    ...
    setCancelling(true)
    // Optimistic update — flip to cancelled immediately so the UI responds instantly
      ...
      <div className="px-5 space-y-3 pb-28">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-20 h-20 rounded-3xl bg-[#0a5048] flex items-center justify-center mb-5 shadow-lg shadow-[#0a5048]/25">
            ...
            <Link
              href="/requests/new"
              className="inline-flex items-center gap-2 bg-[#0a5048] text-white text-sm font-bold px-6 py-3 rounded-2xl shadow-md shadow-[#0a5048]/25 active:scale-[0.97] transition-transform"
            >
...
function PostCard({
  req, applicantCount, cancellingId, cancelling,
  onStartCancel, onCancelAbort, onCancel, t,
}: {
  req: PartnerRequest
    ...
    <div className="bg-white rounded-3xl card-shadow border border-gray-50 overflow-hidden animate-fade-in">
      <div className="px-5 pt-5 pb-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-1.5">
              <svg className="w-3.5 h-3.5 text-[#0a5048]/50 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {req.flexible && (
                <span className="text-xs font-bold text-[#0a5048] bg-[#f0f7f5] px-2 py-0.5 rounded-full">
                  {t.requests.flexible}
                </span>
              )}
            </div>
        ...
        {req.desired_grade_range && (
          <span className="inline-flex mt-2.5 text-[11px] font-bold text-[#0a5048] bg-[#f0f7f5] border border-[#0a5048]/15 px-2.5 py-1 rounded-full">
            {req.desired_grade_range}
          </span>
        )}
      ...
      {isActive && (
        <div className="border-t border-slate-50 px-5 py-3 flex items-center justify-between gap-3">
          {applicantCount > 0 ? (
            <Link
              href="/inbox"
              className="flex items-center gap-2 bg-[#f0f7f5] border border-[#0a5048]/15 text-[#0a5048] text-xs font-bold px-3 py-2 rounded-xl"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0" />
              </svg>
              {applicantCount} {applicantCount === 1 ? t.requests.applicant : t.requests.applicants}
            </Link>
          ) : (
          ...
          {cancellingId === req.id ? (
            <div className="flex items-center gap-2 animate-fade-in">
              ...
              <Button variant="danger" onClick={() => onCancel(req.id)} loading={cancelling} className="!text-xs !px-3 !py-1.5 !rounded-xl">
                {t.requests.yesCancel}
              </Button>
              <Button variant="ghost" onClick={onCancelAbort} className="!text-xs !px-3 !py-1.5 !rounded-xl">
                {t.requests.no}
              </Button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <Link
                ...
                className="text-xs font-bold text-[#0a5048] bg-[#f0f7f5] border border-[#0a5048]/15 px-3 py-2 rounded-xl active:scale-[0.97] transition-transform"
              >
                Edit
              </Link>
              <button
                onClick={() => onStartCancel(req.id)}
