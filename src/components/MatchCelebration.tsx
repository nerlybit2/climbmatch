...
export function MatchCelebration({ result, onClose, closeLabel }: MatchCelebrationProps) {
  ...
  const msg = encodeURIComponent(`Hey! We matched on ClimbMatch at ${result.requestDetails?.location_name || 'the crag'}. Let's plan! 🧗`)
  const whatsappUrl = result.matchedProfile?.phone
    ? `https://wa.me/${result.matchedProfile.phone.replace(/[^0-9]/g, '')}?text=${msg}`
    : null
  const igHandle = result.matchedProfile?.instagram?.replace(/^@/, '').replace(/^(https?:\/\/)?(www\.)?instagram\.com\//, '').replace(/\/$/, '')
  const fbHandle = result.matchedProfile?.facebook?.replace(/^(https?:\/\/)?(www\.)?facebook\.com\//, '').replace(/\/$/, '')
